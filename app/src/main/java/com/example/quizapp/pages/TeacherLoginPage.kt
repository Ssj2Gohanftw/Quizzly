...
import com.example.quizapp.components.LoginProfileAnimation
                    ...
                    databaseRef.addListenerForSingleValueEvent(object : ValueEventListener {
                        override fun onDataChange(snapshot: DataSnapshot) {
                            for (childSnapshot in snapshot.children) {
                                val teacherEmail = childSnapshot.child("email").getValue(String::class.java)
                                if (teacherEmail == userEmail) {
                                    Toast.makeText(context, "Logged in as $teacherEmail", Toast.LENGTH_SHORT).show()
                                    break
                                }
                            }
                        }
...
//        Image(painter = painterResource(id = R.drawable.profile), contentDescription = "Login Image", modifier = Modifier.size(200.dp))
        ...
        Spacer(modifier = Modifier.height(16.dp))
        TextButton(onClick ={
            navController.navigate("trsignup"){
                popUpTo("trlogin"){
                    inclusive=true
                }
            }
